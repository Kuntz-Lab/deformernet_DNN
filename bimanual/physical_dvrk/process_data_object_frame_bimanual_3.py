...
from utils.point_cloud_utils import down_sampling, pcd_ize, world_to_object_frame, object_to_world_frame, find_min_ang_vec, is_homogeneous_matrix, transform_point_cloud, create_open3d_sphere
...
# def world_to_object_frame_PCA(pc):
#     axes, centroid = find_pca_axes(pc)
    
#     # Transpose to get rotation from world to object frame.
#     R_w_o = axes    #np.transpose(axes)
#     d_w_o_o = np.dot(-R_w_o, centroid)
    
#     homo_mat = np.eye(4)
#     homo_mat[:3,:3] = R_w_o
#     homo_mat[:3,3] = d_w_o_o[0]
...
data_processed_path = f"/home/baothach/shape_servo_data/rotation_extension/bimanual_physical_dvrk/multi_{args.obj_category}/processed_data_object_frame_4"
...
with torch.no_grad():
    for i in range(start_index, max_len_data):
        ...
        if vis:
            ...
            # open3d.visualization.draw_geometries([pcd, pcd_goal, coor_1, coor_2, eef_1_sphere, eef_2_sphere,
