...
data_processed_path = f"/home/baothach/shape_servo_data/rotation_extension/bimanual_physical_dvrk/multi_{args.obj_category}/processed_data_object_frame_5_fix_direction"
...
with torch.no_grad():
    for i in range(start_index, max_len_data):
        ...
        mat_1 = compose_4x4_homo_mat(np.eye(3), pos[0][:,0])
        mat_2 = compose_4x4_homo_mat(np.eye(3), pos[1][:,0])
        mat_2 = rotate_around_z(mat_2, np.pi) # IMPORTANT PLEASE EDIT

        # Compute transformation to object frame
        transformation_matrix = world_to_object_frame(pc, mp_pos_1)   #world_to_object_frame(pc, mp_pos_1) np.eye(4)

        new_mp_pos_1 = transform_point_cloud((mp_pos_1 + mat_1[:3,3]).reshape(1, -1), transformation_matrix).flatten()
        new_mp_pos_2 = transform_point_cloud((mp_pos_2 + mat_2[:3,3]).reshape(1, -1), transformation_matrix).flatten()       
        ...
        if vis:
            ...
            new_mani_point_1_sphere = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
            new_mani_point_1_sphere.paint_uniform_color([1,0,1])
            # new_mani_point_1_sphere.translate(tuple(mp_pos_1 + mat_1[:3,3]))
            new_mani_point_1_sphere.translate(tuple(new_mp_pos_1))
            new_mani_point_2_sphere = open3d.geometry.TriangleMesh.create_sphere(radius=0.01)
            new_mani_point_2_sphere.paint_uniform_color([1,1,0])
            # new_mani_point_2_sphere.translate(tuple(mp_pos_2 + mat_2[:3,3]))
            new_mani_point_2_sphere.translate(tuple(new_mp_pos_2))
            
            # line_old_to_new_1 = open3d.geometry.LineSet()
            # line_old_to_new_1.points = open3d.utility.Vector3dVector([mp_pos_1, mp_pos_1 + mat_1[:3,3]])
            # line_old_to_new_1.lines = open3d.utility.Vector2iVector([[0, 1]])
            # line_old_to_new_1.paint_uniform_color([0, 0, 0])
            # line_old_to_new_2 = open3d.geometry.LineSet()
            # line_old_to_new_2.points = open3d.utility.Vector3dVector([mp_pos_2, mp_pos_2 + mat_2[:3,3]])
            # line_old_to_new_2.lines = open3d.utility.Vector2iVector([[0, 1]])
            # line_old_to_new_2.paint_uniform_color([0, 1, 0])
            ...
            open3d.visualization.draw_geometries([pcd, pcd_goal,
                                                ...
                                                new_mani_point_1_sphere, new_mani_point_2_sphere,])
                                                # line_old_to_new_1, line_old_to_new_2])
